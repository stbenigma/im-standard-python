...
def do1diaglink(pdiaglinkxml):
    ...
    diag = Diagram().getbyextref(psrcid=diagguid,psrcname=SOURCE_EAXML)
    if diag is None:
        logmessages.writelog("Diagram {} not found".format(diagguid))
        return
    objguid = handleXML.findRefGuid(pdiaglinkxml, 'ConnectorID')
    rela = Relation().getbyextref(psrcid=objguid,psrcname=SOURCE_EAXML)
    if rela is None:
    ...
    seqNr = 0
    entirefft = Elementrep().getbyuk(eler_diag_id=diag.diag_id,
                                     eler_mode_id=Relation().getbyid(relr.relr_mode_id).rela_enti_id_from)
    entireftf = Elementrep().getbyuk(eler_diag_id=diag.diag_id,
                                     eler_mode_id=Relation().getbyid(relr.relr_mode_id).rela_enti_id_to)
    ...
    startX = entirefft.eler_position_x + (
        entirefft.eler_width / 2 if relr.relr_startedge in (Linesegment.SOUTH, Linesegment.NORTH) \
            else entirefft.eler_width if relr.relr_startedge in (Relationrep.EAST) \
            else 0)
    startY = entirefft.eler_position_y + (
        entirefft.eler_height / 2 if relr.relr_startedge in (Linesegment.EAST, Linesegment.WEST) \
            else entirefft.eler_height if relr.relr_startedge in (Linesegment.SOUTH) \
            else 0)
    endX = entireftf.eler_position_x + (
        entireftf.eler_width / 2 if relr.relr_endedge in (Linesegment.SOUTH, Linesegment.NORTH) \
            else entireftf.eler_width if relr.relr_endedge in (Linesegment.EAST) \
            else 0)
    endY = entireftf.eler_position_y + (
        entireftf.eler_height / 2 if relr.relr_endedge in (Linesegment.EAST, Linesegment.WEST) \
            else entireftf.eler_height if relr.relr_endedge in (Linesegment.SOUTH) \
            else 0)
...
def do1diagobj(pdiagobjxml):
    ...
    enti = Entity().getbyextref(psrcid=objguid,psrcname=SOURCE_EAXML)
    if enti is None:
        logmessages.writelog("Object {} not found for diagram {}".format(objguid, diagguid))
        return
    ...
    for a in attrs:
        atteler = Elementrep()
        atteler.eler_mode_id = a.attr_id
        atteler.eler_diag_id = diag.diag_id
        ...
        atteler.eler_uc = "fillDBea"
        atteler.eler_dc = datetime.today()
        try:
            atteler.insert()
...
def transferobjtypes(proot, pobjtype, ptransferfunc, **restrictions):
    ...
    for obj in objs:
        ...
        #print (handleXML.findColumn(obj, 'Object_ID'))
        restrictionmet = True
...
def do1Attribute(pattrxml):
    ...
    vater = Entity().getbyextref(psrcid=vaterguid,psrcname=SOURCE_EAXML)
    ...
    if domaguid is not None:
        doma = Domain().getbyextref(psrcid=domaguid,psrcname=SOURCE_EAXML)
        if doma is None:
    ...
    if allkeys is not None:
        for key in allkeys.findall('identifier'):
            ...
            if (kr is not None):
                ...
                keys = Key(psrcid=handleXML.findField(key, 'id'), psrcname=SOURCE_EAXML)
...
def transferKeys():
    ...
    for schlentry in schluessel:
        ...
        for ke in reflist:
            ...
            kele.kele_attr_id = Attribute().getIDbyextref(psrcid=ke,psrcname=SOURCE_EAXML)
            if kele.kele_attr_id is None:
                kele.kele_rela_id = Relation().getIDbyextref(psrcid=ke,psrcname=SOURCE_EAXML)
...
def do1Entity(pentitiy):
    ...
    entityguidtransl[entiguid]=entiguid #normal entities are mapped to itself
    entiId = enti.insert()
...
def do1ArcConnector(prelaxml):
    global entityguidtransl,arcboxes
    relaguid = handleXML.findColumn(prelaxml, 'ea_guid')
    stereot = handleXML.findColumn(prelaxml, 'Stereotype')
    conntype = handleXML.findColumn(prelaxml, 'Connector_Type')
    object_id =handleXML.findColumn(prelaxml, 'Connector_ID')
    Start_Object_guid = handleXML.findRefGuid(prelaxml, "Start_Object_ID")
    End_Object_guid = handleXML.findRefGuid(prelaxml, "End_Object_ID")
    if Start_Object_guid in arcboxes:
        entityguidtransl[Start_Object_guid] = End_Object_guid
    elif End_Object_guid in arcboxes:
        entityguidtransl[End_Object_guid] = Start_Object_guid
    else:
        logmessages.writelog(f"Arc2Entity-Connector {relaguid} has no matching ArcBox {Start_Object_guid} or {End_Object_guid}")
    return

def do1Relation(prelaxml):
    global entityguidtransl
    relaguid = handleXML.findColumn(prelaxml, 'ea_guid')
    stereot = handleXML.findColumn(prelaxml, 'Stereotype')
    id = handleXML.findColumn(prelaxml, "Connector_ID")

    srcentiguid = entityguidtransl[handleXML.findRefGuid(prelaxml, "Start_Object_ID")]
    srcenti = Entity().getbyextref(psrcid=srcentiguid,psrcname=SOURCE_EAXML)

    dstentiguid = entityguidtransl[handleXML.findRefGuid(prelaxml, "End_Object_ID")]
    dstenti = Entity().getbyextref(psrcid=dstentiguid,psrcname=SOURCE_EAXML)
    relaname = "RELA-" + id
    ...
    if srccard is None or dstcard is None:
        logmessages.writelog(
