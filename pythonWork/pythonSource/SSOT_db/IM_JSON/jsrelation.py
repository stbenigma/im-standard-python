...
from SSOT_db.IM_JSON import userdefprops, tabreflist
...
#None, "", 'is' in several languages, isa
ROLEASSOCIATIONS= [None] + Relation.ROLEASSOCIATIONS

RELATIONENDMODEL = ['enti',
                    'arc',
                    'assoc',
                    'maptype',
                    'hist',
...
def jsonrelationend(enti, assoc,
                    maptype, hist,
                    mandatory, **kwargs):
    relaend = dict()
    initjselement(model=relaend, refmodel=RELATIONENDMODEL)
    relaend['enti'] = enti
    relaend['assoc'] = assoc
    relaend['maptype'] = maptype
    relaend['hist'] = hist
    relaend['mandatory'] = mandatory
    fillargs(model=relaend, refmodel=RELATIONENDMODEL, **kwargs)
    return relaend


def relaend2js(prelaend=None):
    if prelaend is None:
        retval = fillmodel(pmodel=RELATIONENDMODEL, pentries=['', '', multilangtext(),
                                                              '', '', '', '']
                           )
    else:
        retval = fillmodel(pmodel=RELATIONENDMODEL, pentries=prelaend
                           )
    # fi
    return retval


RELATIONMODEL = ['name', 'type',
                 'from-to', 'to-from',
                 'isinkeys+', 'sourceref',
                 'uc', 'dc', 'um', 'dm',
...
def jsonrelation(name, relatype, uc, dc, relafrom, relato, **kwargs):
    rela = dict()
    initjselement(model=rela, refmodel=RELATIONMODEL)
    rela["name"] = name
    rela["type"] = relatype
    rela["uc"] = uc
    rela["dc"] = dc
    rela["from-to"] = relafrom
    rela["to-from"] = relato
    rela["referencedby"] = []
    rela["userdefprops"] = dict()

    fillargs(model=rela, refmodel=RELATIONMODEL, **kwargs)
    return rela


def relation2js(prela):
    if prela is None:
        retval = fillmodel(pmodel=RELATIONMODEL,
                           pentries=['', '',
        ...
        retval = fillmodel(pmodel=RELATIONMODEL,
                           pentries=[prela.rela_name, prela.rela_type,
                                     relaend2js(prelaend=[jsguid(Modelelemtype.ENTI, prela.rela_enti_id_from),
                                                          None if prela.rela_arcs_id_from is None else jsguid(
                                                              Modelelemtype.ARCS, prela.rela_arcs_id_from),
                                     ...
                                     relaend2js(prelaend=[jsguid(Modelelemtype.ENTI, prela.rela_enti_id_to),
                                                          None if prela.rela_arcs_id_to is None else jsguid(
                                                              Modelelemtype.ARCS, prela.rela_arcs_id_to),
                                     ...
                                     userdefprops(
                                         pprops=udpv2js(pmodeid=prela.rela_id, pmodelemtype=Modelelemtype.RELA)),
                                     [jsguid(Modelelemtype.DOCU, d[0]) for d in
                                      Document.getrefdoculist(pid=prela.rela_id)] \
                                     + [jsguid(Modelelemtype.ORGU, d[0]) for d in
                                        OragnisationalUnit.getreforgulist(pid=prela.rela_id)],
                                     tabreflist(plist={
                                         ...
                                         for s in Datamodel.getmapped(prelaid=prela.rela_id)})
...
def relations2sql(presult: Mergeresult, pjson: JSModel, pwithextsrcref):
    ...
    for jid, jelem in pjson.getelements(pelemtype=Modelelemtype.RELA).items():
        ...
        replacelgtx(presult=presult, pmodeid=newrelaid, pattr=Languagetext.RELA_TEXT_TO,
                    ptexts=jelem['to-from']['assoc'])
        replacelgtx(presult=presult, pmodeid=newrelaid, pattr=Languagetext.RELA_TEXT_FROM,
                    ptexts=jelem['from-to']['assoc'])
...
ARCMODEL = ['name', 'entity',
            'relations+', 'sourceref',
            'uc', 'dc', 'um', 'dm'
            ]

def jsonarc(name,entity,relations,uc,dc,**kwargs):
    arc = dict()
    initjselement(model=arc, refmodel=ARCMODEL)
    arc['name'] = name
    arc['uc'] = uc
    arc['dc'] = dc
    arc['entity'] = entity
    arc['relations+'] = relations
    fillargs(model=arc, refmodel=ARCMODEL, **kwargs)
    return arc

def arcs2js(pemptymodel):
    if pemptymodel:
        retval = {jsguid(Modelelemtype.ARCS, "0000"): fillmodel(pmodel=ARCMODEL,
                                                                pentries=['', '', reflist(), sourceref(), '', '', '',
                                                                          ''])}
    else:
        retval = {jsguid(Modelelemtype.ARCS, a.arcs_id): fillmodel(pmodel=ARCMODEL,
                                                                   pentries=[a.arcs_name,
                                                                             jsguid(Modelelemtype.ENTI, a.arcs_enti_id),
                                                                             [jsguid(Modelelemtype.RELA, r.rela_id) for
                                                                              r in a.getrelalist()],
