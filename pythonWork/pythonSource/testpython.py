...
res = t.render(timestamp=datetime.now()
               ,metainfo = {"title" : "CRM-Salesforce"
                            ...
                            ,}
               ,tables = sorted([[key, value["name"]] for key, value in jsmodel.jsmodel["tables"].items()
                              if value["interface-id"] == intfid],key=lambda x:x[1].upper())
               ,columns = sorted ([[key, value["name"]] for key, value in jsmodel.jsmodel["columns"].items()
                              if value["interface-id+"] == intfid],key=lambda x:x[1].upper())
...
output.write(res)
output.close()
# for domaid,doma in jsmodel.jsmodel["domains"].items():
#     res = t.render(itemid='"{}"'.format(domaid),lang="de", item =doma)
#     print (res)
#     break
