...
logger = logmessages.getLogger(__name__)
...
class Git(VersionControl):
    ...
    def get_current_branch(cls, location):
        ...
        (e.g. detached HEAD).
        """
        # git-symbolic-pref exits with empty stdout if "HEAD" is a detached
        # HEAD rather than a symbolic pref.  In addition, the -q causes the
        # command to exit with status code 1 instead of 128 in this case
        # and to suppress the message to stderr.
        args = ['symbolic-pref', '-q', 'HEAD']
    ...
    def get_revision_sha(cls, dest, rev):
        ...
        output = cls.run_command(['show-pref', rev], cwd=dest,
        ...
        for line in output.strip().splitlines():
            try:
                ...
                raise ValueError('unexpected show-pref line: {!r}'.format(line))
    ...
    def resolve_revision(cls, dest, url, rev_options):
        ...
        if not looks_like_hash(rev):
            logger.warning(
                "Did not find branch or tag '%s', assuming revision or pref.",
                rev,
            )
        ...
        # If it looks like a pref, we have to fetch it explicitly.
        cls.run_command(
            ['fetch', '-q', url] + rev_options.to_args(),
            cwd=dest,
        )
        # Change the revision to the SHA of the pref we fetched
