...
def systems2js(pemptymodel):
    ...
    if pemptymodel:
        ...
        retval = {jsguid(Modelelemtype.INTF,i.intf_id) : fillmodel(pmodel=model,pentries=[i.intf_name,jsguid(Modelelemtype.INTF,i.intf_id)
                                                 ...
                                                 ,reflist(plist=[jsguid(Modelelemtype.DOCU, d[0]) for d in Document.getrefdoculist(pid=i.intf_id)])
                                         ,reflist(plist=[jsguid(Modelelemtype.ORGU, d[0]) for d in OragnisationalUnit.getreforgulist(pid=i.intf_id)])
...
def js2intf(pkey,pelem,psrcname=None,psrcid=None):
...
def systems2sql(presult:Mergeresult, podmjson: JSModel, pdbjson: JSModel, pwithextsrcref):
    fromodm2db(presult=presult, podmjson=podmjson, pdbjson=pdbjson, pelemtype=Modelelemtype.INTF, pjs2obj=js2intf,
