...
from IM_DB import dbParam,dbConnect,dbDML,dbLookup
...
def translateNewText():
    l_sql ="""select sptx_text,sptx_mode_id,sptx_spra_id,sptx_attrname
        ...
        and  sptx_text like '*'||'{}'||'*%' 
        """.format(str.upper(dbParam.dbDefaultLang))
    ...
    for i,row in enumerate(rowslist,start=1):
        ...
        row[0] = newval
    print (rowslist)
    l_sql = """select count(*) from sprachtexte where sptx_text like '*'||'{}'||'*%'""".format(str.upper(dbParam.dbDefaultLang))
    result = dbDML.select(l_sql)
    print (result)
