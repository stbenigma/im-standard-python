...
def putrefinsvg(export: HTMLExport, ptext, pdiagid, plang):
    ...
    for entiid, entival in export.model.getelements(pelemtype='ENTI').items():
        ...
        if ((entisearch.groups() is not None) and (len(entisearch.groups()) > 3)):
            xstart, ystart, xwidth, xoffset = int(entisearch.groups()[0]), int(entisearch.groups()[1]), int(
                entisearch.groups()[2]), int(entisearch.groups()[3])
        ...
        newenti = re.sub('/>.*\n<text id=', '>{}</rect><text id="'.format("<title>{}</title>".format(entidescr)),
                         newenti)
