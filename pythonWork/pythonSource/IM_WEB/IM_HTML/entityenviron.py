...
def printentisvg(pcell:EntityCell, pposx, pposy):
    entitysvg = """<g  fill="{fill}" stroke="{stroke}" fill-opacity="{fillopa}" stroke-opacity="{stropa}" 
            transform="translate({posx},{posy})" >
            <rect x="0" y="0" width="{width}" height="{height}" rx="10" ry="10" /><a href="#{id}" >
            <text id="box{id}" x="6" y="13" fill="{textfill}" font-weight="bold"  fill-opacity="1.0" font-size="{fontsize}" stroke="none">
            {text} </text></a>
            </g>
            """
...
def printlinesvg(pstartx, pstarty, plenx, pleny):
    DEFAULT_LINEWIDTH: int = 1
    line = """<g stroke-linecap="butt" >
              <path stroke="rgb(0,0,0)" fill="none" stroke-opacity="100"  stroke-width="{width}" 
                    d="M{startx} {starty} L{endx} {endy}" />
                </g>
            """
    return line.format(width=DEFAULT_LINEWIDTH,startx=pstartx,starty=pstarty,endx=pstartx+plenx,endy=pstarty+pleny)
...
def generate_svg_content(penviron):
    ...
    for vkey in range(minvidx, maxvidx + 1):
        ...
        if cellleft.gettype() == EntityCell.SUPER:
            svgtext += printentisvg(pcell=cellleft, pposx=entistartx, pposy=entistarty)
            lenx = entistartx+CELLWIDTH-ENTIWIDTH
            if vkey != 0:
                lenx -= LINESHORTEN
            svgtext += printlinesvg(pstartx=entistartx + ENTIWIDTH, pstarty=linestarty, plenx=lenx, pleny=0)
            superlineendy = linestarty

        elif cellcenter.gettype() == EntityCell.PARENT:
            svgtext += printentisvg(pcell=cellcenter, pposx=entistartx, pposy=entistarty)
            linelength = ENTIWIDTH
            svgtext += printlinesvg(pstartx=entistartx + ENTIWIDTH, pstarty=linestarty, plenx=linelength, pleny=0)
        ...
        if cellcenter.gettype() == EntityCell.CENTER:
            ...
            svgtext += printrelasvg (pcell=cellcenter, pposx=entistartx, pposy=relastarty)
        elif cellcenter.gettype() == EntityCell.CHILD:
            svgtext += printrelasvg (pcell=cellcenter, pposx=entistartx+CELLWIDTH-ENTIWIDTH+5, pposy=relastarty)
        else:
            pass
        # fi
        entistartx += CELLWIDTH
        if cellright.gettype() == EntityCell.ROLE:
            svgtext += printentisvg(pcell=cellright, pposx=entistartx, pposy=entistarty)
            lenx = CELLWIDTH-ENTIWIDTH
            if vkey != 0:
                lenx -= LINESHORTEN
            svgtext += printlinesvg(pstartx=entistartx, pstarty=linestarty, plenx=-lenx, pleny=0)
            rolelinestarty = nvl(rolelinestarty,linestarty)
        elif cellcenter.gettype() == EntityCell.CHILD:
            svgtext += printentisvg(pcell=cellcenter, pposx=entistartx, pposy=entistarty)
            linelength = -ENTIWIDTH
            svgtext += printlinesvg(pstartx=entistartx, pstarty=linestarty, plenx=linelength, pleny=0)
