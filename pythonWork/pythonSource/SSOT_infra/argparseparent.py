...
langlist = ','.join(parameters.SUPPORTEDLANGUAGES.keys())
...
_parentparser.add_argument('--paramfile', '-p', dest='paramfile',
                           help=f"Parameterfile for modelenvironent. "
                                + "Default: ./<modelname>{parameters.PARAMFILEEXTENSION}")
...
def fillssotdefaults(pcurrentdir, parguments):
    ...
    parguments['dbtype'] = parameters.SQLITE
    ...
    if argnotnone('modelname', parguments):
        if argisnone('logfile', parguments):
            parguments['logfile'] = os.path.join(pcurrentdir,
                                                 parguments['modelname'] + parameters.LOGFILEEXTENSION)
        if argisnone('paramfile', parguments):
            paramfilepath = os.path.join(pcurrentdir, parguments['modelname'] + parameters.PARAMFILEEXTENSION)
            if os.path.exists(paramfilepath):
                parguments['paramfile'] = paramfilepath
