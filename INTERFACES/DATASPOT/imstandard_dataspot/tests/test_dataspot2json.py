...
class Testjson2dataspot(unittest.TestCase):
    ...
    def test_dataspot2dm(self):
        ...
        dsschema.readmodels(file4)
        self.assertTrue(len(dsschema.tables)>0)


        path1 = Path("dataspottestfiles/Schwipsti")
        ElementId.reset()
        dsschema = d2j.Dataspot2DMJsonschema()
        dsschema.readmodels(path1)
        self.assertTrue(len(dsschema.tables)>0)
    ...
    def test_dataspotelements(self):
        ...
        dsschema = DataspotElements()
        dsschema.readmodels(file1)
        dsschema.readmodels(file2)
        self.assertTrue(len(dsschema.LOVvalues)==0)
        self.assertTrue(len(dsschema.categories)>0)
        self.assertTrue(len(dsschema.domains)>0)

        dsschema = DataspotElements(file3)
        self.assertTrue(len(dsschema.domains) == 0)
        self.assertTrue(len(dsschema.deployments) > 0)
        self.assertTrue(len(dsschema.tables) > 0)
        self.assertEqual(file3.stem ,
                         dsschema.tables[next(iter(dsschema.tables))].get("DSMODEL"))
        self.assertTrue(next(iter(dsschema.tables)).startswith (file3.stem+"/"))

        path1 = Path("dataspottestfiles/Schwipsti")
        dsschema = DataspotElements(path1)
        self.assertTrue(len(dsschema.LOVvalues)>0)
    ...
    def test_localds2json(self):
        ...
        dsschema.readmodels(file1)
        dsschema.readmodels(file2)
        dsschema.readmodels(file3)
        self.assertTrue(len(dsschema.tables)>0)
        self.assertTrue(len(dsschema.columns)>0)
